...
std::string resolve_payload_path(const boost::filesystem::path& plugin_folder, const char* env_name, const char* fallback_name)
{
    if (const char* v = std::getenv(env_name)) {
        boost::filesystem::path p(v);
        if (p.is_absolute())
            return p.string();
        return (plugin_folder / p).string();
    }
...
void LinuxPluginHost::load_modules()
{
    ...
    if (!m_network) {
        const auto path = resolve_payload_path(plugin_folder, "PJARCZAK_BAMBU_NETWORK_SO", "libbambu_networking.so");
        std::string reason;
    ...
    if (!m_source) {
        const auto path = resolve_payload_path(plugin_folder, "PJARCZAK_BAMBU_SOURCE_SO", "libBambuSource.so");
        std::string reason;
        if (manifest_ok && validate_linux_payload_file(path, &reason))
...
nlohmann::json LinuxPluginHost::handle(const std::string& method, const nlohmann::json& payload)
{
    ...
    if (method == "net.get_version") {
        auto f = net<std::string (*)()>("bambu_network_get_version");
        return f ? nlohmann::json{{"ok", true}, {"value", f()}} : not_supported(method);
    }
